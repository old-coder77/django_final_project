...
def submit(request, course_id):
    course =  get_object_or_404(Course, pk=course_id)
    enrollment = Enrollment.objects.filter(user=request.user, course=course).get()
...
def show_exam_result(request, course_id, submission_id):
    ...
    score = 0
    for i in submission.choices.all().filter(selected_correct=True).values_list('question_id'):
